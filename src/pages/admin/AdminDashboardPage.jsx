        ...
        <div className="rounded-[16px] border border-slate-100 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-3">
            {Object.entries(stats?.byStatus || {}).map(([status, count]) => (
              <div
                key={status}
                className="flex items-center justify-between rounded-[10px] bg-[#F8FAFC] px-4 py-3"
              >
                <span className="text-sm font-semibold text-slate-700">
                  {status}
                </span>
                <span className="rounded-full bg-white px-3 py-1 text-xs font-bold text-[#FF6B00]">
                  {count}
                </span>
              </div>
            ))}
            ...
            {Object.entries(stats?.byServiceType || {}).map(
              ([service, count]) => (
                ...
                >
                  ...
                  <span className="rounded-full bg-white px-3 py-1 text-xs font-bold text-[#00AEEF]">
                    {count}
                  </span>
                </div>
              )
